...
class CAMArray:
    ...
    def process_LSH(self):
        ...
        LSH_reference = (LSH_reference > 0).astype(float)
        LSH_reference_comp = np.dot(self.reference_array_comp, self.random_matrix)
        LSH_reference_comp = (LSH_reference_comp > 0).astype(float)
        ...
        reference_array_tensor = torch.tensor(LSH_reference, dtype=torch.int8, device=self.device)
        reference_array_comp_tensor = torch.tensor(LSH_reference_comp, dtype=torch.int8, device=self.device)       
        return reference_array_tensor, reference_array_comp_tensor
