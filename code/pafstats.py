# This file contains code from the open-source project "UNCALLED".
# 
# Project Name: UNCALLED
# Source URL: https://github.com/skovaka/UNCALLED
# License: MIT License
# 
# The code has been adapted and modified from its original form.
# Original Author(s): Sam Kovaka
...
def run(args):
    ...
    if args.ref_paf != None:
        ...
        n = len(locs)
        recall = ntp/(ntp+nfn)
        precision = ntp/(ntp+nfp+nfp_unmap)
        # if (ntp+nfp+nfp_unmap) !=0:
        #     precision = ntp/(ntp+nfp+nfp_unmap)
        # else:
        #     precision = 1
        ...
        statsout.write("Recall: %.2f\n\n" % (100*recall))
        statsout.write("Precision: %.2f\n\n" % (100*precision))
        statsout.write("F1: %.2f\n\n" % (100*F1))
